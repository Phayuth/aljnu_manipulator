...
class HarvesterClient(Node):
    ...
    def __init__(self):
        ...
        self.harvestPoseSubs = self.create_subscription(
            PoseArray,
            self.topicCropHarvestPose,
            self.harvest_pose_array_callback,
            10,
            callback_group=self.callbackGroup,
        )

        # plan and motion
        self.triggerPrePlanner = self.create_service(
            Trigger,
            self.servTriggerPrePlan,
            self.trigger_pre_plan,
            callback_group=self.callbackGroup,
        )
        self.triggerPlanner = self.create_service(
            Trigger,
            self.servTriggerPlan,
            self.trigger_plan,
            callback_group=self.callbackGroup,
        )
        self.triggerMoveForward = self.create_service(
            Trigger,
            self.servExemotionForward,
            self.trigger_execute_forward,
            callback_group=self.callbackGroup,
        )
        self.triggerMoveReverse = self.create_service(
            Trigger,
            self.servExemotionReverse,
            self.trigger_execute_reverse,
            callback_group=self.callbackGroup,
        )
    ...
    def trigger_pre_plan(self, request, response):
        ...
        self.qAux, self.qGoal, response = HarvestBridge.pre_plan_process(
            TCamToBase, TAuxToCam, TGoalToCam, self.ikClient, request, response
        )
        return response

    def trigger_plan(self, request, response):
        self.path, response = HarvestBridge.get_path_planning(
            self.qCurrent, self.qAux, self.qGoal, request, response
        )
        return response

    def trigger_execute_forward(self, request, response):
        trajMsg, response = HarvestBridge.srv_execute_motion_forward(
            self.path, self.jointsName, request, response
        )
        self.trajPublisher.publish(trajMsg)
        return response

    def trigger_execute_reverse(self, request, response):
        trajMsg, response = HarvestBridge.srv_execute_motion_reverse(
            self.path, self.jointsName, request, response
        )
        self.trajPublisher.publish(trajMsg)
        return response
